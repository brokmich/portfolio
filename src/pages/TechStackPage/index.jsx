...
const SKILL_GROUPS = [
  {
    ...
    stripe: '#F4B8D4',
    rotate: '-2.5deg',
    items: ['JavaScript', 'TypeScript', 'HTML', 'CSS / SCSS', 'Angular', 'React', 'Tailwind', 'Bootstrap'],
  },
  {
    category: 'Backend',
    color: '#5A9A25',
  ...
  },
  {
    ...
    stripe: '#E8CA88',
    rotate: '1.2deg',
    items: ['Spanish (native)', 'English (fluent)', 'Python (basic)'],
  },
];
  ...
  return (
    <div
      ...
      style={{
        ...
        transform: hovered ? 'rotate(0deg) scale(1.04)' : `rotate(${group.rotate})`,
    ...
    >
      ...
      <div style={{ padding: '14px 18px 18px' }}>
        <div style={{ fontFamily: "'Cedarville Cursive', cursive", fontSize: 20, color: group.color, marginBottom: 12, lineHeight: 1 }}>
          {group.category}
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {group.items.map((skill, i) => <SkillTag key={i} label={skill} color={group.color} />)}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      <img src="/assets/decoration/silver-star.png" alt="" className="float-b"
        style={{ position: 'absolute', left: 24, top: 24, width: 88, opacity: 0.8 }} />
      <img src="/assets/flowers/green-orchid.png" alt="" className="float-c"
        style={{ position: 'absolute', right: -10, bottom: 0, width: 220, opacity: 0.75 }} />
      <img src="/assets/flowers/pink-lily.png" alt=""
        style={{ position: 'absolute', left: -8, bottom: 10, width: 160, transform: 'rotate(-30deg)', opacity: 0.65 }} />
      ...
      <img src="/assets/decoration/blue-heart.png" alt=""
        style={{ position: 'absolute', left: 60, bottom: 36, width: 82, transform: 'rotate(-12deg)', opacity: 0.75 }} />
      <img src="/assets/about/star-4.png" alt="" className="float-b"
        style={{ position: 'absolute', right: 30, bottom: 60, width: 88, opacity: 0.7 }} />

      <div style={{ zIndex: 2, textAlign: 'center', marginBottom: 18 }}>
        <div style={{ fontFamily: "'Cedarville Cursive', cursive", fontSize: 'clamp(22px,3vw,38px)', color: '#C76D9A', lineHeight: 1.2 }}>
          my tech stack
        </div>
        <div style={{ height: 2, width: 120, background: 'linear-gradient(90deg,transparent,#C76D9A,transparent)', margin: '6px auto 0' }} />
      </div>
      ...
      <div style={{
        ...
        {SKILL_GROUPS.map((g, i) => <StickyNote key={i} group={g} />)}
      </div>
    </div>
  );
}
